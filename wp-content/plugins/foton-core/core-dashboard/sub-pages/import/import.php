...
class FotonCoreImport {
	...
	public function set_import_url() {
		$params = FotonCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
					...
					switch ($_POST['options']['action']):
						...
						case 'rev-slider':
							if (foton_core_is_revolution_slider_installed()) {
								...
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'foton-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!FotonCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									foton_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'foton-core'));
									exit;
								}
							}
			...
			foton_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'foton-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $foton_widget_id => $foton_widget_data) {
				update_option('widget_' . $foton_widget_id, $foton_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'foton-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_options( $file ) {
		...
		$current_options = get_option(FOTON_CORE_OPTIONS_NAME);
		if($options){
			if($current_options != $options) {
				$result = update_option(FOTON_CORE_OPTIONS_NAME, $options);
				if ($result) {
					$this->update_options_after_import($file);
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'foton-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'foton-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'foton-core'));
			}
		}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_foton_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'foton-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'foton-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'foton-core'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'foton-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'foton-core' ) );
		}
	}
	...
	public function import_content( $file, $xml, $attachments, $post_id) {
		ob_start();
		require_once( FOTON_CORE_ABS_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
...
//		}
//
		if(foton_core_is_woocommerce_installed()) {
		...
		$foton_import->fetch_attachments = $attachments;
		$returned_value                  = $foton_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'foton-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'foton-core' ) . ' ' . $xml );

		}

		if($xml == 'foton_content_20.xml') {
	...
	public function rev_sliders() {
		$rev_sldiers = array(
			...
			'marketing-home-1.zip',
			'marketing-home-2.zip',
			'marketing-home-3.zip',
			'marketing-home-4.zip',
			'marketing-home-5.zip',
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'foton-core'));
					exit;
				}
			}
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'foton-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'foton-core'));
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( FOTON_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( FOTON_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function import_get_demo_urls( $folder ) {
		...
		if($folder == 'foton-elementor'){
			$folder = str_replace('-elementor', '', $folder);
			$domain_url = $folder . '.qodeinteractive.com/elementor/';
		} else {
			$folder = str_replace('-wpbakery', '', $folder);
			$domain_url = $folder . '.qodeinteractive.com/';
		}
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return FOTON_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'mkdf_cd_import_nonce' ) ) {
				...
				$html = foton_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					foton_core_ajax_status( 'success', '', $html);
				} else {
					foton_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'foton-core' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = FotonCoreSystemInfoPage::get_instance()->get_system_info();
